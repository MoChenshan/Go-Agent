...
import (
	...
	"git.woa.com/trpc-go/trpc-agent-go/trpc/codeexecutor/pcg123/internal/nfs/errors"
...
// Target represents a mounted NFS export with methods for file operations.
type Target struct {
	*rpc.Client
...
// NewTarget creates a new Target connected to an NFS export at the specified address and port.
// It retrieves filesystem information to optimize read/write operations.
func NewTarget(addr string, port int, auth rpc.Auth, fh []byte, dirpath string) (*Target, error) {
...
// call wraps the RPC Call function to check NFS status and decode errors.
func (v *Target) call(c any) (io.ReadSeeker, error) {
	res, err := v.Call(c)
	if err != nil {
		return nil, err
	}
	...
	if err = errors.NFS3Error(status); err != nil {
		return nil, err
	}
...
// FSInfo retrieves filesystem information from the NFS server.
...
func (v *Target) Lookup(p string) (os.FileInfo, []byte, error) {
	...
	return &namedFileAttr{name: name, Fattr: fattr}, fh, nil
}
...
// ReadDirPlus reads directory entries with attributes and file handles.
// It returns a slice of EntryPlus containing file information.
...
// Create creates a new file with the given path and permissions.
// It returns the file handle of the created file.
