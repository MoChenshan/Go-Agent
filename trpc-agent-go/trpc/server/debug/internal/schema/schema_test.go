...
func TestADKSession_MarshalJSON(t *testing.T) {
	...
	if unmarshaled.LastUpdateTime != 1234567891 {
		t.Errorf(
			"expected LastUpdateTime 1234567891, got %d",
			unmarshaled.LastUpdateTime,
		)
	}
}
...
func TestContent_MarshalJSON(t *testing.T) {
	...
	if unmarshaled.Parts[0].Text != "Hello, world!" {
		t.Errorf(
			"expected first part text 'Hello, world!', got '%s'",
			unmarshaled.Parts[0].Text,
		)
	}
	...
	if unmarshaled.Parts[1].FunctionCall.Name != "test_function" {
		t.Errorf(
			"expected function name 'test_function', got '%s'",
			unmarshaled.Parts[1].FunctionCall.Name,
		)
	}
}
...
func TestAgentRunRequest_MarshalJSON(t *testing.T) {
	...
	if unmarshaled.SessionID != "test-session" {
		t.Errorf(
			"expected SessionID 'test-session', got '%s'",
			unmarshaled.SessionID,
		)
	}
	...
	if unmarshaled.NewMessage.Role != "user" {
		t.Errorf(
			"expected message role 'user', got '%s'",
			unmarshaled.NewMessage.Role,
		)
	}
}
...
func TestFunctionResponse_MarshalJSON(t *testing.T) {
	...
	if unmarshaled.Response != "test response" {
		t.Errorf(
			"expected Response 'test response', got '%v'",
