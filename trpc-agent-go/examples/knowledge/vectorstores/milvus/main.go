...
// Package main demonstrates using Milvus for vector storage.
//
...
//   - MILVUS_DB_NAME: (Optional) Milvus database name
//   - MILVUS_COLLECTION: (Optional) Milvus collection name, defaults to trpc_example
//
// Example usage:
//
...
func main() {
	...
	fmt.Println("🔷 Milvus Vector Store Demo")
	fmt.Println("===========================")

	if address == "" {
		log.Fatal("MILVUS_ADDRESS is required")
	}

	fmt.Printf("📊 Connecting to Milvus: %s\n", address)
