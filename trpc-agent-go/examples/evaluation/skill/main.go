...
var (
	...
	skillsDir = flag.String("skills-dir", "./skills", "Directory containing SKILL.md-based skills")
	...
	evalSetID = flag.String("eval-set", "skill-call-basic", "Evaluation set identifier to execute")
	numRuns   = flag.Int("runs", 1, "Number of times to repeat the evaluation loop per case")
)

const appName = "skill-eval-app"
...
func main() {
	...
	if err != nil {
		log.Fatalf("create skills repo: %v", err)
	}

	r := runner.NewRunner(appName, newSkillAgent(*modelName, *streaming, repo))
	defer r.Close()
...
func printSummary(result *evaluation.EvaluationResult, outDir string) {
	fmt.Println("✅ Evaluation completed with skill call example")
	fmt.Printf("App: %s\n", result.AppName)
