# PromptIter tRPC Server Example

This example exposes the sports-commentary PromptIter workflow through a tRPC `http_no_protocol` control-plane service.

It serves PromptIter through `trpc.group/trpc-go/trpc-agent-go/server/promptiter` and registers the HTTP handler with `git.woa.com/trpc-go/trpc-agent-go/trpc/promptiter`.
...
The listen address is configured by `trpc_go.yaml`. The default service listens on `127.0.0.1:8080` with service name `trpc.test.promptiter.server`.
...
```bash
cd examples/evaluation/promptiter/trpc
