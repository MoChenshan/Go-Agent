...
func main() {
	...
	// 3. 创建 AG-UI 服务，设置自定义 Translator
	...
	if err := tagui.RegisterAGUIServer(server, "trpc.test.react.agui", aguiServer); err != nil {
...
func (t *reactTranslator) handleReceivingEvent(e aguievents.Event) ([]aguievents.Event, error) {
	...
	default:
		log.Warnf("unexpected event type %s while receiving message", e.Type())
		return nil, nil
	}
}
...
func splitTaggedSections(message string) []taggedSection {
	...
	if len(matches) == 0 {
		return []taggedSection{{tag: react.FinalAnswerTag, name: "FINAL_ANSWER", content: message}}
	}
