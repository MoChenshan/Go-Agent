...
import (
	...
	// Import zhiyan for metric and trace OpenTelemetry integration
	_ "git.woa.com/trpc-go/trpc-agent-go/trpc/telemetry/zhiyan"
...
func main() {
	...
	meter := mp.Meter("trpc.agent.go")
	runCount, err := meter.Int64Counter("run", metric.WithDescription("The number of times the iteration ran"))
	if err != nil {
		log.Fatal(err)
	}
	go func() {
		log.Info("start report metric")
		for {
			runCount.Add(context.Background(), 1, metric.WithAttributes(commonAttrs...))
