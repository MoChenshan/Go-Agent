...
func newReviewerAgent(modelInstance model.Model) agent.Agent {
	return llmagent.New(
		...
		llmagent.WithDescription("A dedicated reviewer for unsafe intent decisions."),
...
func newMainAgent(modelInstance model.Model, streaming bool) agent.Agent {
	return llmagent.New(
		mainAgentName,
		llmagent.WithModel(modelInstance),
		llmagent.WithDescription("A chat assistant protected by the unsafe intent guardrail."),
