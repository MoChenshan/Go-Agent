...
func (a *demoApp) setup() error {
	...
	reviewerInstance, err := promptreview.New(a.reviewerRunner)
	if err != nil {
	...
	if err != nil {
		a.reviewerRunner.Close()
		return fmt.Errorf("create prompt injection guardrail: %w", err)
	}
	...
	a.userID = "guardrail-promptinjection-demo-user"
	a.sessionID = fmt.Sprintf("guardrail-promptinjection-demo-session-%d", time.Now().Unix())
	a.printBanner()
	return nil
}
