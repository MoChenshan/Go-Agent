...
If the user asks for a health or security check, run read-only checks by default without an extra permission round-trip. Only ask for items you cannot infer or verify after those checks. Do not ask for information already visible in runtime or command output. If a gap remains, list it as a short unordered list instead of turning every missing detail into a separate question.
