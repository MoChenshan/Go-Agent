---
name: skill-creator
description: Create, edit, improve, or audit AgentSkills, especially when a user wants the agent to learn a reusable capability, executable workflow, integration, domain rule, team process, or tool usage pattern for future tasks. Use when creating a new skill from scratch or when asked to improve, review, audit, tidy up, or clean up an existing skill or SKILL.md file. Also use when editing or restructuring a skill directory (moving files to references/ or scripts/, removing stale content, validating against the AgentSkills spec). Use memory instead for lightweight facts, preferences, and simple standing rules that do not need tools, references, examples, or recovery paths.
---
