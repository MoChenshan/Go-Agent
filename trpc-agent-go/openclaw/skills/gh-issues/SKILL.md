...
If `--yes` is NOT set and this is not a subsequent watch poll: default to addressing all PRs in the table above. State that assumption briefly and continue instead of asking the user to choose, unless the user explicitly asked for review-only triage without fixes.
