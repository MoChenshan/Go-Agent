...
func TestCronTraceAttributesMarkRootObservation(t *testing.T) {
	t.Parallel()

	attrs := cronTraceAttributes(&Job{
		Message: "run report",
		UserID:  "u1",
		Delivery: outbound.DeliveryTarget{
			Channel: "wecom",
	...
	for _, attr := range attrs {
		values[string(attr.Key)] = attr.Value.AsBool()
	}
	require.True(t, values[langfuseInternalAsRoot])
}
