...
const (
	...
	cronTraceInstrumentationName = "trpc.group/trpc-go/trpc-agent-go/" +
		"openclaw/internal/cron"
	cronTraceSpanName = "openclaw.cron"
	...
	langfuseMetadataSource       = langfuseMetadataPrefix + "source"
	langfuseMetadataChannel      = langfuseMetadataPrefix + "channel"
	langfuseMetadataRequestID    = langfuseMetadataPrefix + "request_id"
	langfuseMetadataCronJobID    = langfuseMetadataPrefix + "cron_job_id"
	langfuseMetadataCronJobName  = langfuseMetadataPrefix + "cron_job_name"
	langfuseMetadataDelivery     = langfuseMetadataPrefix + "delivery_target"

	defaultCronTraceOwner = "openclaw"
	cronTraceKind         = "cron"
	traceNameSeparator    = "-"
...
func (s *Service) executeJob(
	...
	runCtx, cronSpan := startCronTraceSpan(
		runCtx,
		job,
		sessionID,
		requestID,
	)
	if trace != nil && cronSpan.SpanContext().IsValid() {
		if err := trace.SetTraceID(
			cronSpan.SpanContext().TraceID().String(),
		); err != nil {
			log.Warnf("cron: persist langfuse trace id failed: %v", err)
		}
	}
	defer func() {
		finishCronTraceSpan(
			cronSpan,
			output,
			firstNonNilError(runErr, deliveryErr),
		)
	}()
	...
	runMessage := model.NewUserMessage(buildScheduledRunMessage(job))
...
func startCronTraceSpan(
	...
	tracer := atrace.TracerProvider.Tracer(cronTraceInstrumentationName)
	ctx, span := tracer.Start(ctx, cronTraceSpanName)
	span.SetAttributes(cronTraceAttributes(job, sessionID, requestID)...)
	return ctx, span
}

func finishCronTraceSpan(span oteltrace.Span, output string, err error) {
	if span == nil {
		return
	}
...
func cronTraceAttributes(
	job *Job,
	sessionID string,
	requestID string,
) []attribute.KeyValue {
	attrs := []attribute.KeyValue{
		attribute.String(langfuseTraceNameAttribute, buildCronTraceName()),
		attribute.Bool(langfuseInternalAsRoot, true),
		attribute.String(langfuseMetadataSource, debugTraceSourceCron),
	}
	...
	if job != nil {
		appendStringAttribute(langfuseTraceInputAttribute, job.Message)
		appendStringAttribute(langfuseObservationInput, job.Message)
		appendStringAttribute(langfuseUserIDAttribute, job.UserID)
		appendStringAttribute(langfuseMetadataChannel, job.Delivery.Channel)
		appendStringAttribute(langfuseMetadataCronJobID, job.ID)
		appendStringAttribute(langfuseMetadataCronJobName, job.Name)
		appendStringAttribute(
			langfuseMetadataDelivery,
			job.Delivery.Target,
		)
	}
	appendStringAttribute(langfuseSessionIDAttribute, sessionID)
	appendStringAttribute(langfuseMetadataRequestID, requestID)
	appendStringAttribute(langfuseMetadataClawID, traceEnvValue(traceNameClawIDEnv))
	return attrs
}

func buildCronTraceName() string {
	owner := firstTraceNameValue(
		traceEnvValue(traceNameClawIDEnv),
		defaultCronTraceOwner,
	)
	return joinTraceName(owner, cronTraceKind)
}
