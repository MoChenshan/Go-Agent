...
func TestLangfuseGatewayTraceAttributes_WeixinRoot(t *testing.T) {
	t.Setenv(langfuseClawIDEnv, "claw-1")

	attrs := langfuseGatewayTraceAttributes(
		&Server{appName: "openclaw"},
		preparedMessageRun{
			userID:    "user-1",
			sessionID: "weixin:dm:bot-1:user-1",
			requestID: "req-1",
			...
			userMsg: model.Message{Content: "fallback"},
		},
	)
	...
	require.Equal(t, "claw-1-wechat", values[langfuseTraceNameAttribute])
	require.Equal(t, "hello", values[langfuseTraceInputAttribute])
	require.Equal(t, "hello", values[langfuseObservationInput])
	require.Equal(t, "user-1", values[langfuseUserIDAttribute])
	require.Equal(
		t,
		"weixin:dm:bot-1:user-1",
		values[langfuseSessionIDAttribute],
	)
	require.Equal(t, "weixin", values[langfuseMetadataChannel])
	require.Equal(t, "req-1", values[langfuseMetadataRequestID])
