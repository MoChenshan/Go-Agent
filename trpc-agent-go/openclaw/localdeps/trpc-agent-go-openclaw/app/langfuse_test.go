...
func TestBuildLangfuseRunOptionResolver_SetsTraceID(t *testing.T) {
	...
	require.Equal(
		t,
		"s1",
		bag.Member(langfuseMetadataTransportSID).Value(),
	)
	require.Equal(
		t,
		"openclaw",
	...
	for _, attr := range opts.SpanAttributes {
		...
		require.Equal(t, "openclaw-wecom-person", attr.Value.AsString())
		foundTraceName = true
	}
...
func TestBuildLangfuseRunOptionResolver_UsesDisplaySessionID(
	t *testing.T,
) {
	t.Parallel()
	...
	ctx, _, err := resolver(context.Background(), gateway.RunOptionInput{
		Inbound: gateway.InboundMessage{
			Channel: "wecom",
		},
		SessionID:  "wecom:dm:T00320026A:1779938395",
	...
	require.Equal(
		t,
		"wecom:dm:zeronezhang:1779938395",
		bag.Member(langfuseSessionIDKey).Value(),
	)
	require.Equal(
		t,
		"wecom:dm:T00320026A:1779938395",
		bag.Member(langfuseMetadataTransportSID).Value(),
	)
}
...
func TestBuildLangfuseRunOptionResolver_UsesRuntimeProfile(
	...
	for _, attr := range opts.SpanAttributes {
		...
		require.Equal(t, "retail-app-wecom-person", attr.Value.AsString())
		foundTraceName = true
	}
	require.True(t, foundTraceName)
}
...
func TestBuildLangfuseTraceName_Fallbacks(t *testing.T) {
	...
	require.Equal(
		...
		"openclaw-wechat",
		buildLangfuseTraceName("", gateway.RunOptionInput{
			Inbound: gateway.InboundMessage{Channel: "weixin"},
		}),
	)
	require.Equal(
		t,
...
func TestBuildLangfuseTraceName_UsesClawIDSuffix(t *testing.T) {
	t.Setenv(langfuseClawIDEnv, "claw-1")

	require.Equal(
		t,
		"claw-1-wecom-group",
		buildLangfuseTraceName("openclaw", gateway.RunOptionInput{
			Inbound:   gateway.InboundMessage{Channel: "wecom"},
			SessionID: "wecom:chat:room-1",
		}),
	)
	require.Equal(
		t,
	...
	require.Equal(
		...
		buildLangfuseTraceName("openclaw", gateway.RunOptionInput{
			Inbound: gateway.InboundMessage{
				Channel: "wecom",
				Thread:  "wecom:dm:user-1",
			},
			SessionID: "wecom:dm:user-1",
		}),
	)
	require.Equal(
		t,
		"claw-1-wechat",
		buildLangfuseTraceName("openclaw", gateway.RunOptionInput{
			Inbound: gateway.InboundMessage{Channel: "wechat"},
		}),
	)
}
